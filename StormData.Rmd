...
Based on the documentations, the k/K, m/M and b/B means thousand, millions and billions.
For numerical values we assumed that they are power of 10. I.E 2 = 10^2, 3 = 10^3 etc.
We will discard other values as we don't know what they might means.
...
ggplot(melted_physical_health, aes(x=EVTYPE, group=variable, fill=as.factor(variable)))+geom_histogram(position="dodge", aes(weight=value))+theme(axis.text.x = element_text(angle=90, hjust=1))+ylab("log scale")+scale_y_log10()+ggtitle("Health Impact Per Type Of Events") + xlab("Event Type")
```
...
ggplot(melted_damages_evtype,aes(x=EVTYPE, group=variable, fill=as.factor(variable)))+geom_histogram(position="dodge",binwidth=2, aes(weight=value))+theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ylab("Costs (log scale)") + scale_y_log10() + ggtitle("Damages Per Type Of Events") + xlab("Event Type")
```
